...
exports.useCoreEdit = exports.useEditOne = exports.useEditOneProps = exports.useEditProps = exports.useEdit = void 0;
...
var ui_alert_1 = require("ui-alert");
var useEdit = function (refForm, initialState, service, p2, p) {
  var router = (0, router_1.useRouter)();
  var baseProps = (0, exports.useCoreEdit)(refForm, initialState, service, p2, p);
  (0, react_1.useEffect)(function () {
    if (refForm) {
      var registerEvents = (p2.ui ? p2.ui.registerEvents : undefined);
      (0, core_1.initForm)(baseProps.refForm.current, registerEvents);
    }
    ...
    if (p && !p.keys && service && service.metadata) {
      ...
      if (metadata) {
        var meta = (0, edit_1.build)(metadata);
    ...
    var id = (0, core_1.buildId)(router.query, keys);
    if (p && p.initialize) {
...
var useEditProps = function (props, refForm, initialState, service, p2, p) {
  var router = (0, router_1.useRouter)();
  var baseProps = (0, exports.useCoreEdit)(refForm, initialState, service, p2, p, props);
  (0, react_1.useEffect)(function () {
    if (refForm) {
      var registerEvents = (p2.ui ? p2.ui.registerEvents : undefined);
      (0, core_1.initForm)(baseProps.refForm.current, registerEvents);
...
exports.useEditProps = useEditProps;
var useEditOneProps = function (p) {
  return (0, exports.useEditProps)(p.props, p.refForm, p.initialState, p.service, p, p);
};
exports.useEditOneProps = useEditOneProps;
var useEditOne = function (p) {
  return (0, exports.useEdit)(p.refForm, p.initialState, p.service, p, p);
};
exports.useEditOne = useEditOne;
var useCoreEdit = function (refForm, initialState, service, p1, p, props) {
  var router = (0, router_1.useRouter)();
  ...
  var _a = (0, react_1.useState)(), running = _a[0], setRunning = _a[1];
  var getModelName = function (f) {
    if (p && p.name && p.name.length > 0) {
      return p.name;
    }
    return (0, core_1.getModelName)(f);
  };
  var baseProps = (0, update_1.useUpdate)(initialState, getModelName, p1.getLocale);
  var state = baseProps.state, setState = baseProps.setState;
  var _b = (0, merge_1.useMergeState)({
    newMode: false,
    setBack: false,
    addable: addable,
  ...
  var _handleNotFound = function (form) {
    var msg = (0, core_1.message)(p1.resource.value, 'error_404', 'error');
    if (form) {
      (0, formutil_1.readOnly)(form);
    }
    p1.showError(msg.message, msg.title, undefined, function () {
      debugger;
      window.history.back;
    });
  };
  ...
  var _createModel = function () {
    ...
    if (metadata) {
      var obj = (0, edit_1.createModel)(metadata);
      return obj;
    }
    else {
      var obj = {};
  ...
  var _onSave = function (isBack) {
    if (flag.newMode === true && flag.addable === false) {
      var m = (0, core_1.message)(p1.resource.value, 'error_permission_add', 'error_permission');
      p1.showError(m.message, m.title);
      return;
    }
    else if (p && flag.newMode === false && p.readOnly) {
      var msg = (0, core_1.message)(p1.resource.value, 'error_permission_edit', 'error_permission');
      ...
      if (flag.newMode) {
        validate(obj_1, function () {
          var msg = (0, core_1.message)(p1.resource.value, 'msg_confirm_save', 'confirm', 'yes', 'no');
        ...
        var diffObj_1 = (0, reflect_1.makeDiff)((0, edit_1.initPropertyNullInModel)(flag.originalModel, metadata), obj_1, keys, version_1);
  ...
  var _validate = function (obj, callback) {
    if (p1.ui) {
      var valid = p1.ui.validateForm(refForm.current, (0, state_1.localeOf)(undefined, p1.getLocale));
      if (valid) {
        callback(obj);
      }
    }
  ...
  var _succeed = function (msg, origin, version, isBack, model) {
    if (model) {
      ...
      if (model && flag.setBack === true) {
        resetState(false, model, (0, reflect_1.clone)(model));
      }
      else {
        (0, edit_1.handleVersion)(origin, version);
      }
    }
    else {
      (0, edit_1.handleVersion)(origin, version);
  ...
  var _fail = function (result) {
    ...
    if (u && f) {
      ...
      (0, formutil_1.focusFirstError)(f);
  ...
  var _postSave = function (r, origin, version, isPatch, backOnSave) {
    ...
    var st = (0, core_1.createEditStatus)(p ? p.status : undefined);
    if (Array.isArray(x)) {
      fail(x);
    }
    else if (!isNaN(x)) {
      if (x === st.success) {
        ...
        if (newMod && x === st.duplicate_key) {
          ...
          (0, edit_1.handleStatus)(x, st, p1.resource.value, p1.showError);
        }
      }
    }
    else {
  ...
  var _handleDuplicateKey = function (result) {
    var msg = (0, core_1.message)(p1.resource.value, 'error_duplicate_key', 'error');
    p1.showError(msg.message, msg.title);
  };
    ...
    if (id && id !== '' && id !== 'new') {
      ...
      service.load(id).then(function (obj) {
        if (!obj) {
          ...
          setFlag({ newMode: false, originalModel: (0, reflect_1.clone)(obj) });
          if (callback) {
  ...
  return __assign(__assign({}, baseProps), { back: back, refForm: refForm, ui: p1.ui, resource: p1.resource.resource(), flag: flag, running: running, setRunning: setRunning, showModel: showModel, getModelName: getModelName, resetState: resetState, handleNotFound: handleNotFound, getModel: getModel, createModel: createModel, create: create, save: save, onSave: onSave, confirm: ui_alert_1.confirm, validate: validate, showMessage: p1.showMessage, succeed: succeed, fail: fail, postSave: postSave, handleDuplicateKey: handleDuplicateKey, load: load, doSave: doSave });
};
exports.useCoreEdit = useCoreEdit;
